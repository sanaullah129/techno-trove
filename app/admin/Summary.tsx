...
import { formatPrice, formatNumber } from '@/utils/formatElements';
...
const Summary: React.FC<SummaryProps> = ({ orders, products, users }) => {
    ...
    useEffect(()=>{
        setSummaryData((prev)=>{
            ...
            const totalSale = orders.reduce((acc, item)=>{
                return acc = item.status === 'completed' ? acc+ item.amount : acc;
            }, 0);
    ...
    //to get keys from objects
    const summaryKeys = Object.keys(summaryData);

    return (
        <div className='max-w-[1150px m-auto]'>
            <div className='mb-4 mt-4'>
                <HeadingDesign title='Statistics' center />
            </div>
